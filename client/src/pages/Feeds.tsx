...
import { ArrowUpRight, Globe, Plus, Trash2, Server, Radio, Network, Settings, Edit, Plane, Ship, Check, Cast, Zap, Activity, Info, BarChart3 } from "lucide-react";
...
const getFeedTypeDetails = (type: string) => {
  switch (type) {
    case "dedicated_app":
      return { label: "Dedicated App", icon: Server, variant: "default" as const, actionLabel: "Manage App", bgClass: "bg-primary/5 border-primary/20" };
    case "integrated_option":
      return { label: "Integrated", icon: Radio, variant: "secondary" as const, actionLabel: "Configure", bgClass: "bg-secondary/10 border-secondary/20" };
    case "raw_stream":
      return { label: "Raw Stream", icon: Network, variant: "outline" as const, actionLabel: "Edit Connection", bgClass: "bg-muted/30 border-border/50" };
    case "audio_stream":
      return { label: "Audio Stream", icon: Cast, variant: "secondary" as const, actionLabel: "Manage Stream", bgClass: "bg-blue-500/5 border-blue-500/10" };
    default:
      return { label: "Unknown", icon: Globe, variant: "secondary" as const, actionLabel: "Manage", bgClass: "bg-muted" };
  }
};
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between px-1">
          <h2 className="text-lg font-semibold tracking-tight">Active Configurations</h2>
        ...
        {mockFeeds.map(feed => {
          ...
          return (
            <Card key={feed.id} className={cn("flex flex-col md:flex-row items-stretch p-0 overflow-hidden border-border/60 transition-all hover:border-primary/50", typeDetails.bgClass)}>
              {/* Left Stripe for visual categorization */}
              <div className={cn("w-1.5 md:h-auto h-1.5 shrink-0", 
                 feed.status === "connected" ? "bg-green-500" : "bg-destructive"
              )}></div>
              ...
              <div className="flex-1 p-5 flex flex-col md:flex-row items-start md:items-center gap-5">
                 ...
                 <div className="flex-1 space-y-1">
                   <div className="flex flex-wrap items-center gap-3">
                     ...
                     <Badge variant={feed.status === "connected" ? "default" : "destructive"} className={cn("shadow-none", feed.status === "connected" ? "bg-green-500/15 text-green-600 hover:bg-green-500/25 border-green-500/20" : "")}>
                       {feed.status}
                     </Badge>
                     <Badge variant={typeDetails.variant} className="gap-1 pl-1.5 font-normal bg-background/50 border-border/50">
                   ...
                   <div className="flex items-center gap-4 text-sm text-muted-foreground">
                     <div className="flex items-center gap-1.5 font-mono">
                       ...
                       {feed.destination}:{feed.port}
                     </div>
                     <span className="text-border">|</span>
                     <span className="text-xs border border-border px-1.5 py-0.5 rounded bg-background/50 font-mono uppercase">{feed.protocol}</span>
                   </div>
                 </div>
              </div>
              ...
              <div className="p-5 flex items-center gap-6 border-t md:border-t-0 md:border-l border-border/50 bg-background/30 backdrop-blur-[2px] w-full md:w-auto justify-between md:justify-end">
                 ...
                 <div className="flex gap-2 pl-6 ml-2 border-l border-border/50">
                   ...
                   <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive hover:bg-destructive/10" title="Remove Feed">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mt-12">
        <div className="rounded-lg bg-card border border-border p-4 flex gap-3 items-start">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">Standalone feeding software managing its own connection (e.g. acarshub).</p>
          </div>
        </div>
        <div className="rounded-lg bg-card border border-border p-4 flex gap-3 items-start">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">Directly built-in feeding capabilities of decoders (e.g. readsb, dump1090).</p>
          </div>
        </div>
        <div className="rounded-lg bg-card border border-border p-4 flex gap-3 items-start">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">Simple TCP/UDP raw data streams to a specific IP:Port destination.</p>
          </div>
        </div>
        <div className="rounded-lg bg-card border border-border p-4 flex gap-3 items-start">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">Icecast mount points for live audio monitoring from radio receivers.</p>
          </div>
        </div>
      </div>
    </div>
