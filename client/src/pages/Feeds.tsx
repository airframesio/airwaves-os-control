...
import { ArrowUpRight, Globe, Plus, Trash2, Server, Radio, Network, Settings, Edit, Plane, Ship, Check, Cast } from "lucide-react";
...
const getFeedTypeDetails = (type: string) => {
  switch (type) {
    ...
    case "audio_stream":
      return { label: "Audio Stream", icon: Cast, variant: "secondary" as const, actionLabel: "Manage Stream" };
    default:
              ...
              <div className="p-4 flex items-center gap-6 border-t md:border-t-0 md:border-l border-border/50 w-full md:w-auto justify-between md:justify-end bg-muted/10 md:bg-transparent rounded-b-lg md:rounded-none">
                 ...
                 <div className="text-center min-w-[80px]">
                   ...
                   <div className="font-medium font-mono text-sm">
                     {feed.type === 'audio_stream' ? 'Live' : `${feed.messageRate}/min`}
                   </div>
                 </div>
                 <div className="flex gap-2 pl-4 border-l border-border/50">
                   ...
                   {feed.type === "audio_stream" && (
                     <Button variant="ghost" size="sm" className="gap-2">
                       <Cast className="w-4 h-4" /> <span className="hidden lg:inline">{typeDetails.actionLabel}</span>
                     </Button>
                   )}
      ...
      <div className="grid grid-cols-1 mt-4">
        <div className="rounded-xl bg-blue-500/5 border border-blue-500/20 p-6">
          <h3 className="font-semibold text-blue-500 mb-2 flex items-center gap-2">
            <Cast className="w-4 h-4" /> Audio Streams
          </h3>
          <p className="text-sm text-muted-foreground">
            Icecast mount points generated by <strong>rtl_airband</strong> for live audio monitoring.
          </p>
        </div>
      </div>
    </div>
